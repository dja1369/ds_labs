...
8. **README 작성 (judgment, 유일한 프로즈 작성 단계)** —
   ...
   만들고 `CONTRACT.md` 3절의 토큰(`TITLE DATE TEMPLATE_NAME TAGS QUESTION HYPOTHESIS
   ...
   **`RESULTS`/`FINDINGS`/`CONCLUSION` 섹션의 모든 수치는 반드시 `results.json`의
   ...
   `NEXT_QUESTIONS`는 형식적 리서치 제안이 아니라 "이거 왜 이런거지?" 톤으로 항목당
   1~2문장, 우선순위 라벨이나 각주식 근거 나열 없이 쓴다.
