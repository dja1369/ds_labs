...
8. **README 작성 (judgment, 유일한 프로즈 작성 단계)** — 먼저 기계적으로 채울 수
   ...
   **`RESULTS`/`FINDINGS`/`CONCLUSION` 섹션의 모든 수치는 반드시 `results.json`의
   ...
   실행 중 웹 대시보드를 제공하면 캡처해 해당 단계에 넣을 수 있다 — **Playwright
   MCP가 아니라 호스트에 직접 설치한 Playwright를 써야 한다**(MCP 브라우저는
   백그라운드 잡에서 Docker 노출 포트에 접근 못 함이 실측 확인됨). 절차는
   `CONTRACT.md`의 "실시간 대시보드 캡처" 절 참고. `FINDINGS`는
