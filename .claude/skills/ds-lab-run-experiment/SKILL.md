...
8. **README 작성 (judgment, 유일한 프로즈 작성 단계)** — 먼저 기계적으로 채울 수
   ...
   **`RESULTS`/`FINDINGS`/`CONCLUSION` 섹션의 모든 수치는 반드시 `results.json`의
   ...
   순서대로 서술하고 각 단계의 실행 커맨드를 코드 블록으로 보여준다. 부하 도구가
   실행 중 웹 대시보드를 제공하면 Playwright MCP(`browser_navigate`/
   `browser_take_screenshot`)로 캡처해 해당 단계에 넣을 수 있다 — 캡처 직전에
   대상이 아직 살아있는지 먼저 확인할 것(부하 실행이 이미 끝나 포트가 닫혀있으면
   접속 실패로 오인하기 쉽다). 절차는 `CONTRACT.md`의 "실시간 대시보드 캡처"
   절 참고. `FINDINGS`는
